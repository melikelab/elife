function [medianVals,h_figure,h_subplots,h_legend,legendStrings] = ...
                     plotXYNresults_eaPDF(files,groups,type,nbinsPDF,...
                                    extractIterStr,iterStrRg,...
...
%       in the legend
%  **optional
% nbinsPDF = number of bins that will be present in the stairs plot of each
%   dataset's PDF
...
% Check inputs
% number of bins
if ~exist('nbinsPDF','var'), nbinsPDF = [];end
% string match for iteration extraction 
if ~exist('extractIterStr','var') || isempty(extractIterStr) || ~ischar(extractIterStr)
    extractIterStr = '_list';
end
% range of text to be extracted
...
%                 'Parent',fg,'XScale','log','XMinorTick','on');
            ...
            if p~= 1
                ...
                set(h_subplots{p,2}(g),'XScale','log','XTickLabel',{'1' '10' '100' '1000'})
            end
...
%             end
%             % new using stairs (December 2015)
%             if f == size(filegroup,1)
%                 bindata = cell2mat( aggData{g}.(dataTypes{p,1}) );
...
%                     'LineStyle','-');%linestyl{f});
%             end
            % make stairs PDF for each dataset (January 21, 2016)
            [bins,ct] = plotStairs(currData,nbinsPDF,'log');
            stairs(h_subplots{p,2}(g),bins,ct,...
                'Color',colrs(g,:),...
                'LineStyle',linestyl{f});
            ...
            legendStrings{m} = [type{g} ' - ' ...
                    filegroup{f,1}(numbridx+iterStrRg(1):numbridx+iterStrRg(2))];
        end
