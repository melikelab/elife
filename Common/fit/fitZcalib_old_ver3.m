...
defaultVals = { 'ascending';   ... stageMovement
                 1.5;          ... mSigma
              };
[stageMovement,mSigma] ...
...
%     else
    ...
    while tstfr < size(stageData,1)
        if steps(tstfr) ~= 0 
            if steps(tstfr) > 0 && ~stageMovement.userInput % do not adjust value if the user provided the input
                stageMovement.value = 'ascending'; % else stick with default/input
            end
            break
        end
    	tstfr = tstfr+1;
        ...
        if ~exist('stageMovement','var') || isempty(stageMovement)
            ...
            if isempty(vals{1}) && isempty(vals{2})
                ...
                while isempty(in)
                    ...
                    if in == 1
                        stageMovement.value = 'ascending';
                        stageMovement.userInput = true;
                    elseif in == 2
                        stageMovement.value = 'decending';
                        stageMovement.userInput = true;
                    else
                        in = [];
                    end
                ...
                stageMovement.value = stageMovement;
                stageMovement.userInput = true;
            end
        end
